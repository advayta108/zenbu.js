...
**Kernel shortcut:** `zen kyju generate` from `packages/init/`
...
**Location:** `~/.zenbu/registry/`, generated by `zen link`
...
Running `zen link` from any plugin directory:
...
Run `zen link` from a plugin directory after:
...
2. Run `zen kyju generate --name add_my_field` from `packages/init/`
...
6. Run `zen link` from the plugin directory to update registry types
...
3. **Registry-based types**: `~/.zenbu/registry/services.ts` imports the actual service classes and exports a `ServiceRouter` type using a mapped `ExtractRpcMethods<T>` utility type. This file is generated by running `zen link` from a plugin directory. Go-to-definition follows through to the actual service method implementations.
...
After adding a service, re-run `zen link` from the plugin directory to update the registry types.
...
3. Run `zen link` from the plugin directory to add its types to the registry
...
| Command | What it does |
|---------|--------------|
| `zen` | Open a new Zenbu window (connects to running instance over socket, or spawns the Electron app). |
...
Zenbu downloads its own `bun` and `pnpm` on first launch rather than bundling them inside the `.app` or relying on the user's toolchain. This follows the opencode pattern — bundle the core runtime, lazy-install specific tools — and the Volta/asdf/mise pattern of "I manage my own toolchain, coexisting with yours."

### Storage layout

- **`~/.zenbu/`** — user source + config. `plugins/zenbu/` is the git clone; `config.json` lists installed plugin manifests; `bin/zen` is the CLI shim; `.internal/paths.json` is written by the kernel on every launch with the resolved cache paths.
- **`~/Library/Caches/Zenbu/`** (via `app.getPath("cache")`) — regenerable toolchain tree. Can be deleted and the next launch re-downloads everything.
  - `bin/bun`, `bin/pnpm` — downloaded binaries, pinned versions in `setup/versions.json`, SHA256-verified.
  - `bun/` — `BUN_INSTALL` root (install cache, globals).
  - `pnpm/` — `PNPM_HOME`.
  - `xdg/cache`, `xdg/data`, `xdg/state` — redirect pnpm's `cacheDir`, `storeDir`/`globalDir`/`globalBinDir`, and `stateDir` respectively (per [pnpm docs](https://pnpm.io/settings)). So our pnpm's store never collides with the user's at `~/Library/pnpm/`.

### Isolation env vars (set by `bootstrapEnv` in `apps/kernel/src/shell/env-bootstrap.ts`)

```
BUN_INSTALL=~/Library/Caches/Zenbu/bun
PNPM_HOME=~/Library/Caches/Zenbu/pnpm
XDG_CACHE_HOME=~/Library/Caches/Zenbu/xdg/cache
XDG_DATA_HOME=~/Library/Caches/Zenbu/xdg/data
XDG_STATE_HOME=~/Library/Caches/Zenbu/xdg/state
...
`ZENBU_BUN`, `ZENBU_PNPM`, `ZENBU_GIT` environment variables (if set when Zenbu launches) win over the downloaded binaries. Useful if you want to test against a specific bun version you already have.

### Hard user-system deps

Zenbu assumes only:
- **Xcode Command Line Tools** (provides `git` + `bash`). Preflight detects via `xcode-select -p` / git-stub behavior and offers `xcode-select --install`.

No requirement for `node`, `bun`, or `pnpm` on the user's system.

### Preflight screens

Two screens may appear before the main setup window:
