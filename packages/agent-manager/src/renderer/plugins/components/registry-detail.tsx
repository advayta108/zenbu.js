...
import { streamdownProps } from "#zenbu/init/src/renderer/views/chat/lib/streamdown-config"
...
// Right column. Display surface for `PluginEntry` + lazy `RepoInfo` +
// lazy `ReadmeState`; actions bubble up to the parent's state machine.
...
function derivePrimaryAction(
  ...
  if (activity && !activity.done) {
    if (activity.kind === "install")
      return { kind: "disabled", label: "Installing…" }
    if (activity.kind === "update")
      return { kind: "disabled", label: "Updating…" }
    if (activity.kind === "uninstall")
      return { kind: "disabled", label: "Removing…" }
  }
...
function DetailMenu({
  ...
  return (
    <>
      ...
      />
      <div className="absolute right-0 top-[32px] z-50 min-w-[200px] rounded-md border border-black/10 bg-white py-1 text-[12px] shadow-md">
        {entry.repo && (
